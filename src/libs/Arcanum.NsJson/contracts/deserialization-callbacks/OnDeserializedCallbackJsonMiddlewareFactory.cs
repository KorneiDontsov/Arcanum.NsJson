...
namespace Arcanum.NsJson.Contracts {
	...
	public class OnDeserializedCallbackJsonMiddlewareFactory: IJsonMiddlewareFactory {
		...
		class Middleware: IFromJsonMiddleware {
			Action<Object> onDeserialized { get; }

			public Middleware (Action<Object> onDeserialized) =>
				this.onDeserialized = onDeserialized;

			/// <inheritdoc />
			public Object? Read (JsonReader reader, JsonSerializer serializer, ReadJson next) {
				var data = next(reader, serializer);
				if (data is {}) onDeserialized(data);
				return data;
			}
		}
		...
		public void Handle (IJsonMiddlewareRequest request) {
			...
			if (callbacks.Count > 0) {
				...
				request.Yield(new Middleware(onDeserialized));
			}
		}
	}
}
