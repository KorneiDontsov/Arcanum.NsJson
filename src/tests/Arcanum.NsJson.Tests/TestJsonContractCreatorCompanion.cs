...
namespace Arcanum.NsJson.Tests {
	...
	public class TestJsonContractCreatorCompanion: TestJsonSerializer {
		class MutDnsEndPoint {
			...
			[UsedImplicitly]
			public class Companion: IJsonContractCreator {
				/// <inheritdoc />
				public JsonContract CreateJsonContract () {
					var hostProp =
						new JsonProperty {
							PropertyName = "Host",
							UnderlyingName = nameof(maybeHost),
					...
					var portProp =
						new JsonProperty {
							PropertyName = "Port",
							UnderlyingName = nameof(maybePort),
